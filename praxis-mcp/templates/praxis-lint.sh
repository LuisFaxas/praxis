...
check_structure() {
    ...
    if [ -d "$dev_dir/work-orders/_executed" ]; then
        emit "S-006" "structure" "pass" "work-orders/_executed/"
    elif [ -d "$dev_dir/work-orders/executed" ]; then
        emit "S-006" "structure" "warn" \
            "Legacy completed queue detected: work-orders/executed/" \
            "Rename to work-orders/_executed/ (preferred naming)"
    else
        emit "S-006" "structure" "fail" "Missing: work-orders/_executed/" \
            "mkdir -p $dev_dir/work-orders/_executed"
        FIXABLE_DIRS+=("$dev_dir/work-orders/_executed")
    fi
    ...
    if [ "$tier" != "starter" ]; then
        ...
        if [ -d "$dev_dir/commands/_executed" ]; then
            emit "S-012" "structure" "pass" "commands/_executed/"
        elif [ -d "$dev_dir/commands/executed" ]; then
            emit "S-012" "structure" "warn" \
                "Legacy completed queue detected: commands/executed/" \
                "Rename to commands/_executed/ (preferred naming)"
        else
            emit "S-012" "structure" "warn" "Missing: commands/_executed/" \
                "mkdir -p $dev_dir/commands/_executed"
            FIXABLE_DIRS+=("$dev_dir/commands/_executed")
        fi
    fi
    ...
    if [ "$DETECTED_MODE" = "triangle" ]; then
        ...
        for agent_dir in "$dev_dir/work-orders"/wo_*/; do
            ...
            if [ -d "$agent_dir/_executed" ]; then
                emit "S-TRI" "structure" "pass" "$agent_name/_executed/"
            elif [ -d "$agent_dir/executed" ]; then
                emit "S-TRI" "structure" "warn" \
                    "Legacy completed queue detected: $agent_name/executed/" \
                    "Rename to $agent_name/_executed/ (preferred naming)"
            else
                emit "S-TRI" "structure" "warn" "Missing: $agent_name/_executed/" \
                    "Create agent folder: mkdir -p dev/work-orders/$agent_name/_executed"
            fi
        done
        if [ "$found_agent_dirs" -eq 0 ]; then
            emit "S-TRI" "structure" "warn" "Triangle mode detected but no wo_*/ folders found" \
                "Create agent folders: mkdir -p dev/work-orders/wo_{agent}/_executed"
        fi
    fi
}
...
check_work_orders() {
    ...
    # Internal: validate patch WO metadata + parent linkage
    _check_patch_wo() {
        local wo_file="$1"
        local filename="$2"
        local search_dir="$3"
        ...
        # {parentNumber}_{YYYY-MM-DD}_{DESCRIPTION}_P{NN}.md
        if [[ "$filename" =~ ^([0-9]+)_[0-9]{4}-[0-9]{2}-[0-9]{2}_.+_P([0-9]{2})\.md$ ]]; then
            local parent_num="${BASH_REMATCH[1]}"
            local patch_num="${BASH_REMATCH[2]}"

            # Metadata fields
            if ! grep -qiE "^\s*-\s*\*\*Parent WO:\*\*" "$wo_file" 2>/dev/null; then
                emit "WO-005" "work_order_integrity" "warn" \
                    "${label_prefix}${filename} — patch WO missing '**Parent WO:**' metadata" \
                    "Add '- **Parent WO:** ${parent_num}' to WO header"
            fi

            if ! grep -qiE "^\s*-\s*\*\*Patch:\*\*" "$wo_file" 2>/dev/null; then
                emit "WO-005" "work_order_integrity" "warn" \
                    "${label_prefix}${filename} — patch WO missing '**Patch:**' metadata" \
                    "Add '- **Patch:** ${patch_num}' to WO header"
            fi

            if ! grep -qiE "^\s*-\s*\*\*Sequence Key:\*\*" "$wo_file" 2>/dev/null; then
                emit "WO-005" "work_order_integrity" "warn" \
                    "${label_prefix}${filename} — patch WO missing '**Sequence Key:**' metadata" \
                    "Add '- **Sequence Key:** ${parent_num}.${patch_num}' to WO header"
            fi

            # Parent WO exists in the same queue lane (excluding patch files)
            local parent_found=0
            for parent in "$search_dir/${parent_num}_"*.md; do
                [ -f "$parent" ] || continue
                local parent_base
                parent_base=$(basename "$parent")
                [[ "$parent_base" == *"_P"[0-9][0-9]".md" ]] && continue
                parent_found=1
                break
            done

            if [ "$parent_found" -eq 0 ]; then
                emit "WO-006" "work_order_integrity" "fail" \
                    "${label_prefix}${filename} — patch parent WO ${parent_num} not found in lane" \
                    "Create/restore parent WO ${parent_num}_*.md in this lane before using patch WOs"
            else
                emit "WO-006" "work_order_integrity" "pass" \
                    "${label_prefix}${filename} — patch parent WO ${parent_num} found"
            fi
        fi
    }

    # Internal: check patch sequence continuity per parent (P01, P02, ...)
    _check_patch_sequence() {
        local dir="$1"
        local label_prefix="${2:-}"
        local had_patch=0

        shopt -s nullglob
        local patch_files=("$dir"/*_P[0-9][0-9].md)
        shopt -u nullglob

        [ ${#patch_files[@]} -eq 0 ] && return 0

        had_patch=1

        # Collect parent numbers
        local parents
        parents=$(for wo in "${patch_files[@]}"; do
            local f
            f=$(basename "$wo")
            if [[ "$f" =~ ^([0-9]+)_[0-9]{4}-[0-9]{2}-[0-9]{2}_.+_P([0-9]{2})\.md$ ]]; then
                echo "${BASH_REMATCH[1]}"
            fi
        done | sort -u)
        ...
        while IFS= read -r parent; do
            ...
            local seq
            seq=$(for wo in "$dir"/"${parent}"_*_P[0-9][0-9].md; do
                [ -f "$wo" ] || continue
                local f
                f=$(basename "$wo")
                if [[ "$f" =~ _P([0-9]{2})\.md$ ]]; then
            ...
            while IFS= read -r n; do
                ...
                if [ "$n" -ne "$expected" ]; then
                    any_gap=1
                    emit "WO-007" "work_order_integrity" "warn" \
                        "${label_prefix}patch sequence gap for parent ${parent}: expected P$(printf '%02d' "$expected"), found P$(printf '%02d' "$n")" \
                        "Renumber patch files to continuous sequence P01, P02, ... for parent ${parent}"
                    break
                fi
                expected=$((expected + 1))
            done <<< "$seq"
        ...
        if [ "$had_patch" -eq 1 ] && [ "$any_gap" -eq 0 ]; then
            emit "WO-007" "work_order_integrity" "pass" \
                "${label_prefix}patch sequence continuity valid (P01..Pn)"
        fi
    }

    # Internal: resolve completed queue dir (prefers _executed, supports legacy executed)
    _resolve_exec_dir() {
        local base="$1"
        if [ -d "$base/_executed" ]; then
            echo "$base/_executed"
        elif [ -d "$base/executed" ]; then
            echo "$base/executed"
        else
            echo "$base/_executed"
        fi
    ...
    # Internal: resolve lane completed queue dir.
    # Canonical: wo_{agent}/_executed/{lane}/
    # Legacy compatibility: wo_{agent}/{lane}/_executed/ or executed/
    _resolve_lane_exec_dir() {
        local agent_dir="$1"
        local lane_dir="$2"
        local lane_name="$3"

        if [ -d "$agent_dir/_executed/$lane_name" ]; then
            echo "$agent_dir/_executed/$lane_name"
        elif [ -d "$lane_dir/_executed" ]; then
            echo "$lane_dir/_executed"
        elif [ -d "$lane_dir/executed" ]; then
            echo "$lane_dir/executed"
        else
            echo "$agent_dir/_executed/$lane_name"
        fi
    }
    ...
    _check_executed_dir() {
        ...
        for wo in "$exec_dir"/*.md; do
            ...
            found_any=1
            local unchecked
            unchecked=$(grep -c "\- \[ \]" "$wo" 2>/dev/null || true)
            unchecked="${unchecked:-0}"

            # N/A criteria recognition: - [ ] ~~text~~ N/A — reason
            local na_count
            na_count=$(grep -cE "^- \[ \] ~~.+~~[[:space:]]*N/A[[:space:]]*[—–-]" "$wo" 2>/dev/null || true)
            na_count="${na_count:-0}"

            # Subtract N/A from unchecked (N/A are resolved, not blocking)
            unchecked=$((unchecked - na_count))
            [ "$unchecked" -lt 0 ] && unchecked=0

            _check_patch_wo "$wo" "$filename" "$(dirname "$wo")" "$label_prefix"

            if [ "$unchecked" -gt 0 ]; then
                if grep -qiE "^\s*-\s*\*\*Status:\*\*\s*Superseded|^\s*Status:\s*Superseded" "$wo" 2>/dev/null; then
                    emit "WO-001" "work_order_integrity" "warn" \
                        "${label_prefix}${filename} is superseded with ${unchecked} unchecked criterion in ${exec_label}/" \
                        "Optional cleanup: mark superseded criteria as N/A or add supersession note in Acceptance Criteria"
                else
                    emit "WO-001" "work_order_integrity" "fail" \
                        "${label_prefix}${filename} has ${unchecked} unchecked criterion in ${exec_label}/" \
            ...
            # WO-012: N/A guardrails (executed WOs)
            if [ "$na_count" -gt 3 ]; then
                emit "WO-012" "work_order_integrity" "fail" \
                    "${label_prefix}${filename} has ${na_count} N/A criteria (max 3)" \
                    "WO is poorly scoped — split into smaller WOs"
            elif [ "$na_count" -gt 1 ]; then
                emit "WO-012" "work_order_integrity" "warn" \
                    "${label_prefix}${filename} has ${na_count} N/A criteria" \
                    "Consider tighter scoping for future WOs"
            elif [ "$na_count" -gt 0 ]; then
                emit "WO-012" "work_order_integrity" "info" \
                    "${label_prefix}${filename} — ${na_count} criterion marked N/A" ""
            fi
        done

        if [ "$found_any" -eq 0 ]; then
            emit "WO-001" "work_order_integrity" "info" \
                "${label_prefix}${exec_label}/ has no non-example WOs"
        fi
    }
    ...
    _check_active_wo() {
        ...
        for wo in "$active_dir"/*.md; do
            ...
            # WO-013: N/A criteria in active WOs (should rewrite, not mark N/A)
            local active_na
            active_na=$(grep -cE "^- \[ \] ~~.+~~[[:space:]]*N/A[[:space:]]*[—–-]" "$wo" 2>/dev/null || true)
            active_na="${active_na:-0}"
            if [ "$active_na" -gt 0 ]; then
                emit "WO-013" "work_order_integrity" "warn" \
                    "${label_prefix}${filename} has ${active_na} N/A criteria in active WO" \
                    "Rewrite criteria instead of marking N/A in active WOs"
            fi

            _check_patch_wo "$wo" "$filename" "$active_dir" "$label_prefix"
        done

        _check_patch_sequence "$active_dir" "$label_prefix"
    }
    ...
    if [ "$DETECTED_MODE" = "triangle" ]; then
        ...
        for agent_dir in "$wo_dir"/wo_*/; do
            ...
            local agent_name
            agent_name=$(basename "$agent_dir")
            local agent_exec_dir
            agent_exec_dir=$(_resolve_exec_dir "$agent_dir")
            _check_executed_dir "$agent_exec_dir" "$agent_name/"
            _check_active_wo "$agent_dir" "$agent_name/"

            # Nested lanes under each agent (for example: 10_delivery_academy/, 80_lab_.../, etc.)
            local lane_dir
            for lane_dir in "$agent_dir"/*/; do
                [ -d "$lane_dir" ] || continue
                local lane_name
                lane_name=$(basename "$lane_dir")
                local lane_type=""
                [ "$lane_name" = "executed" ] && continue
                [ "$lane_name" = "_executed" ] && continue

                if [[ "$lane_name" =~ ^[0-9]{2}_(delivery|program|lab|ops)_[a-z0-9_]+$ ]]; then
                    lane_type="${BASH_REMATCH[1]}"
                else
                    emit "WO-008" "work_order_integrity" "warn" \
                        "$agent_name/$lane_name — lane folder does not match {nn}_{type}_{scope} naming convention" \
                        "Rename lane to format like 10_delivery_academy"
                fi

                if ! compgen -G "${lane_dir}0_*_README.md" > /dev/null; then
                    emit "WO-009" "work_order_integrity" "warn" \
                        "$agent_name/$lane_name — missing lane README (0_*_README.md)" \
                        "Add lane README documenting scope, active chain, deferred chain, and cadence"
                fi

                local lane_exec_dir
                lane_exec_dir=$(_resolve_lane_exec_dir "$agent_dir" "$lane_dir" "$lane_name")

                if [ -d "$lane_dir/_executed" ]; then
                    emit "WO-011" "work_order_integrity" "warn" \
                        "$agent_name/$lane_name — legacy lane-local _executed/ detected" \
                        "Migrate completed files to $agent_name/_executed/$lane_name/"
                fi
                if [ -d "$lane_dir/executed" ]; then
                    emit "WO-011" "work_order_integrity" "warn" \
                        "$agent_name/$lane_name — legacy lane-local executed/ detected" \
                        "Migrate completed files to $agent_name/_executed/$lane_name/"
                fi

                _check_executed_dir "$lane_exec_dir" "$agent_name/$lane_name/"
                if [ "$lane_type" = "program" ] || [ "$lane_type" = "lab" ]; then
                    emit "WO-010" "work_order_integrity" "info" \
                        "$agent_name/$lane_name — $lane_type lane detected; active files treated as planning/lab docs (Acceptance Criteria and Status optional)"
                    _check_patch_sequence "$lane_dir" "$agent_name/$lane_name/"
                else
                    _check_active_wo "$lane_dir" "$agent_name/$lane_name/"
                fi
            done
        done
        if [ "$found_agent_wo" -eq 0 ]; then
            ...
            local root_exec_dir
            root_exec_dir=$(_resolve_exec_dir "$wo_dir")
            _check_executed_dir "$root_exec_dir" ""
...
# --- Category 7: Orphan Detection (OD-001 to OD-007) ---
...
check_orphans() {
    ...
    # OD-005: Screenshot/image files at project root (must live in dev/design/audit/screenshots/)
    for file in \
        "$PRAXIS_TARGET_DIR"/*.png \
        "$PRAXIS_TARGET_DIR"/*.jpg \
        "$PRAXIS_TARGET_DIR"/*.jpeg \
        "$PRAXIS_TARGET_DIR"/*.webp \
        "$PRAXIS_TARGET_DIR"/*.gif; do
        [ -f "$file" ] || continue
        local filename
        filename=$(basename "$file")
        found_orphans=1
        emit "OD-005" "orphan_detection" "fail" \
            "Screenshot/image at project root: $filename" \
            "Move to dev/design/audit/screenshots/{date}_{topic}/"
    done

    # OD-006: Screenshot/image files under dev/private/ (must live in dev/design/audit/screenshots/)
    if [ -d "$dev_dir/private" ]; then
        while IFS= read -r -d '' img; do
            local relative="${img#$PRAXIS_TARGET_DIR/}"
            found_orphans=1
            emit "OD-006" "orphan_detection" "fail" \
                "Screenshot/image in dev/private/: $relative" \
                "Move to dev/design/audit/screenshots/{date}_{topic}/"
        done < <(find "$dev_dir/private" -type f \( \
            -iname "*.png" -o -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.webp" -o -iname "*.gif" \
        \) -print0 2>/dev/null)
    fi

    # OD-007: Screenshot/image files under .playwright-mcp/ temp folder
    if [ -d "$PRAXIS_TARGET_DIR/.playwright-mcp" ]; then
        while IFS= read -r -d '' img; do
            local relative="${img#$PRAXIS_TARGET_DIR/}"
            found_orphans=1
            emit "OD-007" "orphan_detection" "warn" \
                "Screenshot/image in temp tooling folder: $relative" \
                "Move to dev/design/audit/screenshots/{date}_{topic}/ and clean temp artifacts"
        done < <(find "$PRAXIS_TARGET_DIR/.playwright-mcp" -type f \( \
            -iname "*.png" -o -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.webp" -o -iname "*.gif" \
        \) -print0 2>/dev/null)
    fi

    # OD-008: Screenshot/image files directly in screenshots/ root (must be in dated subfolders)
    local screenshots_root="$dev_dir/design/audit/screenshots"
    if [ -d "$screenshots_root" ]; then
        for file in \
            "$screenshots_root"/*.png \
            "$screenshots_root"/*.jpg \
            "$screenshots_root"/*.jpeg \
            "$screenshots_root"/*.webp \
            "$screenshots_root"/*.gif; do
            [ -f "$file" ] || continue
            local filename
            filename=$(basename "$file")
            found_orphans=1
            emit "OD-008" "orphan_detection" "fail" \
                "Screenshot/image in screenshots/ root: $filename" \
                "Move to dev/design/audit/screenshots/{date}_{topic}/(run_*)/"
        done
    fi
